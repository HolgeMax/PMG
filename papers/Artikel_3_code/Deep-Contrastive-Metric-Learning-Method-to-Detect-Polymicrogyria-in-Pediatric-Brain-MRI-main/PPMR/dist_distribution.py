...
def evaluation_roc(test_filepaths, test_labels, tag_name):
    ...
    normal_dist = dists[np.where((labels == 0))]
    abnormal_dist = dists[np.where((labels == 1))]

    normal_dist = np.squeeze(normal_dist)
    abnormal_dist = np.squeeze(abnormal_dist)
    ...
    plt.hist(abnormal_dist, **kwargs, color="b", histtype="step")
    plt.hist(abnormal_dist, **kwargs, color="b", label="seen anomaly")
    plt.gca().set(
        title=tag_name + " data prediction",
    ...
    plt.plot(x, stats.kde.gaussian_kde(abnormal_dist)(x) * 1000, "b", linewidth=2)

    plt.plot(x, stats.kde.gaussian_kde(normal_dist)(x) * 1000, "g", linewidth=2)

    plt.axvline(x=5.0, color="k")
    plt.legend()

    # plt.show()
    plt.savefig(tag_name + "_distance.png")
...
# python3 dist_distribution.py --test="13 23 34 6" --val="31 29 3 28" --inner_fold=1
