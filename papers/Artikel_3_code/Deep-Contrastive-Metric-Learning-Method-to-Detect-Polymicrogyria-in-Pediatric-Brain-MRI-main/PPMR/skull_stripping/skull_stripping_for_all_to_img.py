    ...
    for patient_id in tqdm(os.listdir(base_dir)):
        ...
        if whether_normal == True:
            for control_id in os.listdir(patient_id_folder):
                ...
                save_control_path_saved = os.path.join(
                    save_patient_id_folder, control_id
                )
                ...
                for slice_ in range(nii_data.shape[0]):
                    ...
                    Image.fromarray(nii_data[slice_, :, :]).convert("RGB").save(
                        os.path.join(save_control_path_saved, image_name_array[slice_])
                    )
            ...
            if normal_abnormal[0].__contains__("cor"):
                ...
                save_coronal_path_saved = os.path.join(
                    save_patient_id_folder, normal_abnormal[0]
                )
                ...
                save_coronal_path_saved = os.path.join(
                    save_patient_id_folder, normal_abnormal[1]
                )
            ...
            for slice_ in range(nii_data.shape[0]):
                ...
                Image.fromarray(nii_data[slice_, :, :]).convert("RGB").save(
                    os.path.join(save_coronal_path_saved, image_name_array[slice_])
                )
...
save_base_dir = (
    "/home/lingfeng/Desktop/Nested_CV/thesis_plots_drawing/skull_stripping_images"
)
