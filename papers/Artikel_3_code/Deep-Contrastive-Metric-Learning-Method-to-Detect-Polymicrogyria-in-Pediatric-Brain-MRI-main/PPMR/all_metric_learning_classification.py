...
class model_class:
    def dilated_block(self, x, kernel_size, channel):
        ...
        dil_1 = Conv2D(
            channel, (kernel_size, kernel_size), padding="same", dilation_rate=(1, 1)
        )(x)
        dil_1 = BatchNormalization()(dil_1)
        dil_1 = tf.nn.leaky_relu(dil_1, alpha=leaky_relu_alpha)

        dil_2 = Conv2D(
            channel, (kernel_size, kernel_size), padding="same", dilation_rate=(2, 2)
        )(x)
        dil_2 = BatchNormalization()(dil_2)
        dil_2 = tf.nn.leaky_relu(dil_2, alpha=leaky_relu_alpha)

        dil_3 = Conv2D(
            channel, (kernel_size, kernel_size), padding="same", dilation_rate=(3, 3)
        )(x)
...
def one_patient_dataset_collection_func(i):  # or add some patients' normal slices
    test_filepaths = []
    test_labels = []
    ...
    for patient_item in tqdm(natsorted(os.listdir(dataset_dir))):
        ...
        for slice_num in range(len(coronal_path_list)):
            ...
            if patient_item == str(test_value[i]):
                test_filepaths.append(
                    os.path.join(coronal_path_saved, coronal_path_list[slice_num])
                )
                test_labels.append(get_label(coronal_path_list[slice_num]))
    ...
    return test_filepaths, test_labels


def one_control_dataset_collection_func(i):  # or add some patients' normal slices
    print("test_value[i]:", test_value[i])
    ...
    return test_filepaths, test_labels, num_list


def one_patient_evaluation(test_filepaths, test_labels):
    checkpoint.restore(ckpt_manager.latest_checkpoint)
    ...
    return labels, predictions, dists


for i in range(4):
    test_filepaths, test_labels = one_patient_dataset_collection_func(i)
    patient_labels, patient_predictions, patient_dists = one_patient_evaluation(
        test_filepaths, test_labels
    )

    test_filepaths, test_labels, num_list = one_control_dataset_collection_func(i)
    control_labels, control_predictions, control_dists = one_patient_evaluation(
        test_filepaths, test_labels
    )

    # print("test_filepaths:",test_filepaths)
    # print("labels:",labels)
    # print("predictions:",predictions)
    # print("dists:",dists)

    # plt.bar(np.arange(len(dists)),dists,width=1.)
    # # specifying horizontal line type
    # plt.axhline(y = 5, color = 'r', linestyle = '-')
    # plt.savefig("distance.png")

    dists_concate = patient_dists + control_dists

    color_list = []

    for num in patient_labels:
        if num == 1:
            color_list.append("red")
        else:
            color_list.append("green")

    for _ in range(len(control_labels)):
        color_list.append("green")

    plt.figure()

    plt.ylim((0, 25))
    plt.xlabel("MRI slices")
    plt.ylabel("Predicted distance")

    plt.bar(np.arange(len(dists_concate)), dists_concate, color=color_list, width=1.0)
    # specifying horizontal line type
    plt.axhline(y=5, color="blue", linestyle="-")

    plt.axvline(x=len(patient_dists), color="black", linestyle="-")

    plt.axvline(x=len(patient_dists) + num_list[0], color="black", linestyle="-")

    plt.axvline(
        x=len(patient_dists) + num_list[0] + num_list[1], color="black", linestyle="-"
    )

    plt.axvline(
        x=len(patient_dists) + num_list[0] + num_list[1] + num_list[2],
        color="black",
        linestyle="-",
    )

    plt.savefig("results_distance" + str(i) + ".png")

# python3 all_metric_learning_classification.py --test="13 23 34 6" --val="31 29 3 28" --inner_fold=1
