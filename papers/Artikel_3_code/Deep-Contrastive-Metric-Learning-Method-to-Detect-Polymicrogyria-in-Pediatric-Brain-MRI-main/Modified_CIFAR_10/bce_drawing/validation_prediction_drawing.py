...
def dataset_collection_func(
    normal_class, abnormal_classes_list, abnormal_ratio=0.1, normal_amount=5000
):
...
def evaluation(test_images, test_labels):
    ...
    print("probability[np.where((labels==0))]:", probability[np.where((labels == 0))])

    plt.hist(probability[np.where((labels == 0))], **kwargs, color="g", label="normal")
    plt.hist(
        probability[np.where((labels == 1))], **kwargs, color="b", label="abnormal"
    )
    plt.gca().set(
        title="BCE loss validation data prediction",
        ylabel="Amount",
    ...
    plt.legend()
    # plt.show()
    plt.savefig("validation_bce_loss.png")
