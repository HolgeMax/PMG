...
def evaluation(test_images, test_labels):
    ...
    unseen_abnormal_dist = unseen_abnormal_dist[
        np.where((unseen_abnormal_dist <= 0.01))
    ]
    unseen_abnormal_mu, unseen_abnormal_std = norm.fit(unseen_abnormal_dist)

    seen_abnormal_dist = seen_abnormal_dist[np.where((seen_abnormal_dist <= 0.01))]
    seen_abnormal_mu, seen_abnormal_std = norm.fit(seen_abnormal_dist)

    normal_dist = normal_dist[np.where((normal_dist <= 0.01))]
    normal_mu, normal_std = norm.fit(normal_dist)

    plt.hist(normal_dist, **kwargs, color="g", label="normal")
    plt.hist(seen_abnormal_dist, **kwargs, color="b", label="seen abnormal")
    plt.hist(unseen_abnormal_dist, **kwargs, color="r", label="unseen abnormal")
    plt.gca().set(
    ...
    # plt.legend()
    # plt.show()
    plt.savefig("bce_loss_copy_left.png")
