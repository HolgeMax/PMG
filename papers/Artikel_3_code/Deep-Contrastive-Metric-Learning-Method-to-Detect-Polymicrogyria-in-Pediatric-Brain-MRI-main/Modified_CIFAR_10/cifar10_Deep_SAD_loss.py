...
(
    ...
    normal_class=0,
    abnormal_classes_list=[1, 2, 3, 4, 5],
    abnormal_ratio=0.1,
    normal_amount=5000,
)
...
checkpoint_dir = os.path.join(base_dir, "Deep_SAD_loss_classification")
...
def compute_loss(labels, z, eps=1e-4):
    ...
    abnormal_dist_loss = abnormal_labels * (1.0 / (dist + eps))
...
def valid_step(inputs, labels, R):
    # print("validation......")
    ...
    for epoch in range(epochs):
        ...
        for index in range(0, len(train_labels) - BATCH_SIZE, BATCH_SIZE):
            ...
            if iteration % 30 == 0:
                ...
                R = get_R_based_on_c(train_images, train_labels, nu)
...
def evaluation(test_images, test_labels, nu):
    print("testing")
...
evaluation(validation_images, validation_labels, nu)
evaluation(test_images, test_labels, nu)
