"""Logic for running the black-box occlusion ablation study.

The CLI wrapper in ``src/cli/ablation.py`` only holds the Hydra decorator;
...
def run_ablation(cfg: DictConfig) -> None:
    """Run the ablation study and write results to CSV and YAML.

    Args:
        cfg: Hydra config containing ``ablation``, ``data_loader``, and
            ``train`` sub-configs.
    """
    device = (
